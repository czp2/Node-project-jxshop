    ...
    $.get('http://kg.zhaodashen.cn/v1/category/index.jsp', {}, res => {
        ...
        let html = ``
			$.each(res.data, (index, menu) => {
				...
				html +=
					`<h3><a href="${menu.url ? menu.url : './list.html?k='+menu.keywords}">${menu.cat_name}</a> <b></b></h3>`
				...
				$.each(menu.children, (index, menu2) => {
					...
					html +=
						`<dt><a href="${menu2.url ? menu2.url : './list.html?k='+menu2.keywords}">${menu2.cat_name}</a></dt>`
					html += `<dd>`
					// 三级
					$.each(menu2.children, (index, menu3) => {
						html +=
							`<a href="${menu3.url ? menu3.url : './list.html?cat='+menu3.cat_id}">${menu3.cat_name}</a>`
					})
					html += `</dd>`
					html += `</dl>`
				})
